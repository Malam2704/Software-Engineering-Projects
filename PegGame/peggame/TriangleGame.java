...
public class TriangleGame implements PegGame{
    // Parameters
    private GameState state;
    protected PegGame game;
    private TriangleBoard board;

    public TriangleGame(TriangleBoard board) {
        this.state = GameState.NOT_STARTED;
        this.board = board;
    }

    private TriangleGame(PegGame copyGame, TriangleBoard copyBoard, GameState copyState) {
    ...
    public void moverFunction(List<Move> possibleMoves, Location from){
        for(int toRow=0; toRow < board.getMyCoordinates().length; toRow++) {
            for(int toCol=0; toCol < board.getMyCoordinates()[toRow].length; toCol++) {
    ...
    public Collection<Move> getPossibleMoves() {
        ...
        for(int row=0; row < board.getMyCoordinates().length; row++) {
            for(int col=0; col < board.getMyCoordinates()[row].length; col++) {
    ...
    public PegGame deepCopy() {
        PegGame copy = new TriangleGame(this.game, this.board, this.getGameState());
        return copy;
    }
}
